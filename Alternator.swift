...
struct Project {
  nonisolated(unsafe) static var src: URL?
  nonisolated(unsafe) static var tgt: URL?
...
struct File {
  ...
  var contents: String { get throws {
    ...
    if src.pathExtension == "md"
      { return MarkdownParser.shared.html(from: text) }

    guard try !metadata.isEmpty
    else { return text }
  ...
  var ref: String
    { src.rawPath.replacingFirst(of: Project.src!.path()).ref }
  ...
  func build() throws {
    ...
    if isRenderable {
      print("[build] rendering \(src.masked) -> \(tgt.masked)",
        to: &FileHandle.stderr)
      FileManager.default
        .createFile(atPath: tgt.rawPath, contents: try render().data(using: .utf8))
    } else {
      print("[build] copying \(src.masked) -> \(tgt.masked)", to: &FileHandle.stderr)
      try src.touch()
  ...
  func render(_ context: [String: String] = [:]) throws -> String {
    var context = context
      .merging(try metadata, uniquingKeysWith: { (x, _) in x })
...
struct Layout {
  ...
  func render() throws -> String {
    ...
    let pattern =
      #"((<!--|/\*|/\*\*)\s*#content[\s\S]*?(-->|\*/)|//[^\S\r\n]*#content[^\n]*)"#
    for match in text.find(pattern)
...
struct FSDiffStream {
  let callback: FSEventStreamCallback = { (_, _, numEvents, eventPaths, _, _) in
    ...
    let diffs = (0..<numEvents)
      .map { URL(
        fileURLWithFileSystemRepresentation: bufferPointer[$0],
        isDirectory: false,
        relativeTo: nil) }
      .unique
  ...
  @discardableResult public init() {
    ...
    print("[watch] watching \(Project.src!.masked) for changes",
      to: &FileHandle.stderr)
  }
}
...
final class HTTPServer: Sendable {
  ...
  struct Response {
    ...
    var data: Data {
      ...
      headerLines
        .append(contentsOf: headers.map { "\($0.key.rawValue): \($0.value)" })
  ...
  @discardableResult init(port: UInt16) {
    ...
    listener.newConnectionHandler = { (_ conn) in
      conn.start(queue: .main); self.receive(from: conn) }
    listener.start(queue: .main)

    print("[serve] serving \(Project.tgt!.masked) at http://localhost:\(port)",
      to: &FileHandle.stderr)
  }
  ...
  func receive(from conn: NWConnection) {
    ...
    { content, _, complete, err in
      if let err
        { print("[serve] Error: \(err)", to: &FileHandle.stderr) }
...
extension URL {
  ...
  var modificationDate: Date? { get throws {
    try FileManager.default
      .attributesOfItem(atPath: rawPath)[FileAttributeKey.modificationDate] as? Date
  }}
